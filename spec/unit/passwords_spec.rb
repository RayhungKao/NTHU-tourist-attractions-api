...
describe 'Test Password Digestion' do
  it 'SECURITY: create password digests safely, hiding raw password' do
    ...
    digest = NTHUtouristAttractions::Password.digest(password)
  ...
  it 'SECURITY: successfully checks correct password from stored digest' do
    password = 'secret password of 雷松亞 stored in db'
    digest_s = NTHUtouristAttractions::Password.digest(password).to_s

    digest = NTHUtouristAttractions::Password.from_digest(digest_s)
  ...
  it 'SECURITY: successfully detects incorrect password from stored digest' do
    ...
    digest_s1 = NTHUtouristAttractions::Password.digest(password1).to_s

    digest1 = NTHUtouristAttractions::Password.from_digest(digest_s1)
